...
var pinchGesture = {
    ...
    touchstart: function (event) {
        ...
        if (Object.keys(gestures).length === 2) {
            ...
            gestures.elements = elements
            fireGesture(getCommonAncestor(elements), 'pinchstart', {
    ...
    touchmove: function (event) {
        var elements = gestures.elements || []
        if (elements.length === 2) {
            ...
            fireGesture(getCommonAncestor(elements), 'pinch', {
            ...
            if (transform.scale > 1) {
                fireGesture(getCommonAncestor(elements), 'pinchout', {
                    transform: transform,
                    touches: event.touches,
                    touchEvent: event
                })
                ...
                fireGesture(getCommonAncestor(elements), 'pinchin', {
                    transform: transform,
                    touches: event.touches,
                    touchEvent: event
                })
    ...
    touchend: function (event) {
        ...
        if (elements.length === 2) {
            ...
            fireGesture(getCommonAncestor(elements), 'pinchend', {
                touches: event.touches,
                touchEvent: event
            })
        }
        endGesture(event, noop)
    }
}

pinchGesture.touchcancel = pinchGesture.touchend

avalon.gestureHooks.add('pinch', pinchGesture)